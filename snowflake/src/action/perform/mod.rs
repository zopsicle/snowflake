...
pub type Result<'a> =
    std::result::Result<Summary<'a>, Error>;
...
/// Perform an action.
pub fn perform<'a>(perform: &Perform, action: &'a Action) -> Result<'a>
{
    match action {
...
fn perform_create_symbolic_link(perform: &Perform, target: &CStr)
    -> Result<'static>
{
    let output = Path::new("output");
    symlinkat(target, Some(perform.scratch), output)?;
    Ok(Summary{outputs: vec![output.into()], warnings: false})
}
